...
EXPERT_PROFILES = {"baseline": (8, 768), "narrow24": (24, 256), "narrow48": (48, 128)}


def pretrain_recipe(*, profile="baseline", cp=8, dp=1, experts=None, width=None, top_k=2):
    if cp * dp != 8 or (cp, dp) not in ((8, 1), (4, 2), (2, 4)):
        raise ValueError("supported v5e-8 attention layouts: CP8/DP1, CP4/DP2, CP2/DP4")
    e, f = EXPERT_PROFILES[profile]
    config = apply_tokenizer_contract(ModelConfig())
    config = replace(
        config, n_experts=e if experts is None else experts,
        d_ff=f if width is None else width, experts_per_token=top_k,
