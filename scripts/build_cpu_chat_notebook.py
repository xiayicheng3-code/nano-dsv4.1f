...
def build_notebook() -> nbf.NotebookNode:
    ...
    nb.cells = [
        ...
        nbf.v4.new_code_cell(
            """# A small multi-turn chat session using DeepSeek V4.1 Chat Completions formatting.\n"
            "import json\n\n"
            "history = []\n\n"
            "def reset_chat():\n"
            "    history.clear()\n\n"
            "def chat(message, *, thinking=False, reasoning_effort=50, max_tokens=128):\n"
        ...
        nbf.v4.new_markdown_cell(
            """## Chat interactively\n\n"
            "After **Run All** finishes, add or reuse a cell and call:\n\n"
            "```python\n"
            "print_reply(chat('Why is sparse attention useful?'))\n"
            "print_reply(chat('Now explain that more simply.'))  # keeps the same history\n"
            "```\n\n"
            "For DeepSeek-style reasoning mode:\n\n"
            "```python\n"
            "print_reply(chat('Solve 37*43 carefully.', thinking=True, reasoning_effort=75, max_tokens=256))\n"
            "```\n\n"
            "Use `reset_chat()` to start a fresh conversation. The notebook intentionally "
            "uses ordinary executable cells rather than depending on Kaggle widget support."
            """
        ),
        nbf.v4.new_code_cell(
